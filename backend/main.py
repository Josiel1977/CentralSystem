...
# Habilita CORS total para Vercel e chamadas remotas
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Resolução dinâmica do caminho do index.html para o ambiente Linux da Vercel
...
def renderizar_index(request: Request):
  ...
  if not os.path.exists(index_path):
    return HTMLResponse(
        content=(
            "<h1>Deploy Ativo no Vercel</h1><p>API operando normalmente."
            " Acesse /docs para ver a documentação das rotas.</p>"
        ),
        status_code=200,
    )
